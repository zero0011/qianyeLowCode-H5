...
type dictType = 'String' | 'Array' | 'Object' | 'Boolean' | 'Number';


let getElementConfig = function (element: any, extendStyle = {}) {
	let elementData = cloneDeep(element)
	let type: dictType = elementData.valueType || 'String' // 默认string类型
	let dict = {
		'String': '',
		'Array': [],
		'Object': {},
...
let copyElement = function(element: any, extendStyle = {}){
...
let copyPage = function(data: any){
	let pageData = cloneDeep(data);
	pageData.uuid = createUUID();
	pageData.elements = pageData.elements.map((element: any) => {
